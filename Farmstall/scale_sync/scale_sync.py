...
and pushes price updates directly to the BC-4000 scale via TCP port 7061.
...
DEAD_LETTER_SKIP_FAILURES = 5       # skip for 1h after this many
DEAD_LETTER_SKIP_HOURS = 1
DEAD_LETTER_PERMANENT_FAILURES = 24 # require manual intervention after this many
...
def _atomic_write(path: Path, data: str):
    """Write atomically via temp file + rename (safe on Linux/ext4)."""
...
def is_dead_letter(product: dict, failed: dict) -> bool:
    """Return True if this product should be skipped this cycle."""
    ...
    if skip_until:
        if datetime.now(timezone.utc) < datetime.fromisoformat(skip_until):
            return True
        # Skip window expired — allow retry
        rec.pop('skip_until', None)
    return False


def mark_failed(product_id: int, error: str, failed: dict):
    """Record a failure; escalate to dead-letter if threshold reached."""
    key = str(product_id)
    ...
    if n >= DEAD_LETTER_PERMANENT_FAILURES:
        logger.error(
            f"Product {product_id} has failed {n} times — PERMANENT dead-letter. "
            f"Manual intervention required."
        )
        rec['permanent'] = True
    elif n >= DEAD_LETTER_SKIP_FAILURES:
        skip_until = (
            datetime.now(timezone.utc) + timedelta(hours=DEAD_LETTER_SKIP_HOURS)
        ).isoformat()
        rec['skip_until'] = skip_until
        logger.warning(
            f"Product {product_id} skipped until {skip_until} after {n} failures "
            f"— scale may have stale price"
        )


def reset_failed(product_id: int, failed: dict):
    """Clear failure record after a successful send."""
...
def fetch_products(conn, since: Optional[str]):
    """
    Fetch products changed since `since` (or all active if since is None).
    Returns (rows, watermark) where watermark = max(updated_at) of the result set.
    Both computed inside a single transaction for snapshot consistency.
    """
...
# ---------------------------------------------------------------------------
# Scale connectivity pre-check
# ---------------------------------------------------------------------------

def scale_reachable() -> bool:
    try:
        s = socket_connect_only()
        s.close()
        return True
    except Exception as e:
...
def run_sync_cycle(conn):
    ...
    for chunk_idx, chunk in enumerate(chunked(to_sync, CHUNK_SIZE)):
        if is_first_run:
            time.sleep(1)  # throttle initial full sync
        ...
        if result is None:
            # All retries failed — mark each product in chunk as failed
            for p in products_in_chunk:
                mark_failed(p['id'], str(last_exc), failed)
            save_failed(failed)
            logger.error(
                f"Chunk {chunk_idx} failed after 3 retries: {last_exc} — "
                f"not advancing watermark"
            )
            write_heartbeat()
        ...
        # Reset failure counts for successfully sent products
        for p in products_in_chunk:
            reset_failed(p['id'], failed)

        new_watermark = chunk_watermark if new_watermark is None else max(new_watermark, chunk_watermark)

        # Persist watermark after EACH successful chunk for crash safety.
        # Do not remove — this is required by the 5-second overlap window correctness.
        save_state({'last_sync_watermark': new_watermark.isoformat()})
    ...
    logger.info(
        f"Sync cycle complete: sent={total_sent} updated={total_updated} "
...
def main():
    ...
    logger.info(f"Scale sync service starting")
    ...
    while True:
        try:
            ...
            # Attempt to reconnect DB on next cycle
            try:
                conn.close()
            except Exception:
                pass
